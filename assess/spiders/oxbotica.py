...
class OxboticaSpider(scrapy.Spider):
    name = 'oxbotica'

    def start_requests(self):
        yield scrapy.Request(
            url='https://apply.workable.com/api/v1/widget/accounts/317876?origin=embed&callback=whrcallback',
            callback=self.parse,
            dont_filter=True,
        )
    ...
    def parse(self, response, **kwargs):
        ...
        for job in jobs:
            yield scrapy.Request(
                url=f'https://apply.workable.com/api/v2/accounts/oxbotica/jobs/{job["shortcode"]}',
